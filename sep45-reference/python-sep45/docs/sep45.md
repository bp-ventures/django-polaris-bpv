...
# Local Reference: Anchor Platform Implementation

The content above this line is the upstream SEP-45 specification copied verbatim
from `stellar/stellar-protocol`. The content below is local engineering notes
mapping the spec to the Java/Kotlin Anchor Platform reference implementation
checked out at `/home/antb2/dev/rsync/anchor-platform/`. Use it to locate the
canonical end-to-end flow when designing the Python (django-polaris) equivalent.

Every path below is absolute and verified to exist on this machine (paths and
line numbers checked 20260514).

## Auth handshake (SEP-45 challenge + token)

| Concern                          | Anchor Platform location                                                                                                              |
|----------------------------------|---------------------------------------------------------------------------------------------------------------------------------------|
| HTTP controller                  | `/home/antb2/dev/rsync/anchor-platform/platform/src/main/java/org/stellar/anchor/platform/controller/sep/Sep45Controller.java`        |
| Challenge + verify service       | `/home/antb2/dev/rsync/anchor-platform/core/src/main/java/org/stellar/anchor/sep45/Sep45Service.java`                                 |
| Config interface                 | `/home/antb2/dev/rsync/anchor-platform/core/src/main/java/org/stellar/anchor/config/Sep45Config.java`                                 |
| Config impl (Spring properties)  | `/home/antb2/dev/rsync/anchor-platform/platform/src/main/java/org/stellar/anchor/platform/config/PropertySep45Config.java`            |
| Bean wiring                      | `/home/antb2/dev/rsync/anchor-platform/platform/src/main/java/org/stellar/anchor/platform/component/sep/SepBeans.java`                |
| JWT model (SEP-45)               | `/home/antb2/dev/rsync/anchor-platform/core/src/main/java/org/stellar/anchor/auth/Sep45Jwt.java`                                      |
| Shared web-auth JWT base         | `/home/antb2/dev/rsync/anchor-platform/core/src/main/java/org/stellar/anchor/auth/WebAuthJwt.java`                                    |
| Downstream JWT filter (10 + 45)  | `/home/antb2/dev/rsync/anchor-platform/core/src/main/java/org/stellar/anchor/filter/WebAuthJwtFilter.java`                            |
| Nonce store (replay protection)  | `/home/antb2/dev/rsync/anchor-platform/core/src/main/java/org/stellar/anchor/auth/NonceManager.java` + JDBC persistence               |
| Client-domain signer lookup      | `/home/antb2/dev/rsync/anchor-platform/core/src/main/java/org/stellar/anchor/util/ClientDomainHelper.java`                            |
| Simulation backend dependency    | `StellarRpc` — required for `web_auth_verify` simulation in both recording and enforcing modes                                        |

Request flow (`/home/antb2/dev/rsync/anchor-platform/core/src/main/java/org/stellar/anchor/sep45/Sep45Service.java`):

```
GET  /sep45/auth   →  validate (account, home_domain)
                  →  build SCVal args for web_auth_verify
                  →  simulate InvokeHostFunctionOperation in recording mode
                  →  extract auth entries from sim result
                  →  sign server-account entry with SIGNING_KEY
                  →  return { authorization_entries, network_passphrase }

POST /sep45/auth  →  decode submitted auth entries
                  →  assert entries share identical args
                  →  verify home_domain / web_auth_domain / client_domain*
                  →  verify server-account signature
                  →  atomically consume nonce
                  →  re-simulate in enforcing mode with submitted entries
                  →  issue SEP-45 JWT
```

The shared `WebAuthJwtFilter` decodes SEP-10 first, then SEP-45 — downstream
SEP-6/24/31/38 controllers receive a common authenticated session object
regardless of which auth flow produced it. Reuse this pattern in Python: a
web-auth abstraction above SEP-10 and SEP-45, with a common session object on
the request.

## Payment integration (deposits + observation)

SEP-45 only issues the JWT. The actual on-chain movement happens through
SEP-6/24/31/38 flows authenticated by that JWT. Two halves:

### Send (anchor → user, deposit payout)

Single path, always Soroban SAC.transfer — no classic PaymentOp send.

`/home/antb2/dev/rsync/anchor-platform/kotlin-reference-server/src/main/kotlin/org/stellar/reference/client/PaymentClient.kt:31-41`:
...
`sendToAccount`
(`/home/antb2/dev/rsync/anchor-platform/kotlin-reference-server/src/main/kotlin/org/stellar/reference/client/PaymentClient.kt:51-132`)
hard-wraps non-null memo into a muxed M-address:
...
|-------------|-------------------------------------------------------------------------------|
| `C…`        | Memo dropped. Contract id itself is the routing key.                          |
| `G…`        | Numeric only. Rewrapped into `M(G, u64)` so the u64 rides in the SAC event.   |
| `M…`        | Forwarded as-is.                                                              |

Text memos are not supported on Soroban deposits by the reference server.

### Observe (user → anchor, deposit ingestion)

Dual-tracked:

- Classic txs ingested by `HorizonPaymentObserver`
  (`/home/antb2/dev/rsync/anchor-platform/platform/src/main/java/org/stellar/anchor/platform/observer/stellar/HorizonPaymentObserver.java`),
  reading the Horizon `payments_for_account` stream — full text / id / hash memo support.
- Soroban txs ingested by `StellarRpcPaymentObserver`
  (`/home/antb2/dev/rsync/anchor-platform/platform/src/main/java/org/stellar/anchor/platform/observer/stellar/StellarRpcPaymentObserver.java`),
  watching SAC `transfer` events.

Both feed a common `PaymentTransferEvent { from, to, amount, memo, asset }`.

The Soroban observer's CAP-0067 decode lives at
`/home/antb2/dev/rsync/anchor-platform/platform/src/main/java/org/stellar/anchor/platform/observer/stellar/StellarRpcPaymentObserver.java:200-254`:

- Topics: `(Symbol("transfer"), Address from, Address to, String asset)`
- Data: either `SCV_I128 amount`, or `SCV_MAP { amount: i128, memo: ... }`
- The memo SCVal is type-switched:
...
Send side never emits text or bytes — but observe side decodes them if a
third-party wrapper contract emits them.

## End-to-end correlation pattern

For a SEP-45-authenticated user depositing into an anchor hotwallet, the
canonical flow is:

```
user (Soroban) --[SAC.transfer to M(hotwallet_G, sep24_memo_id), no tx memo]--> hotwallet
hotwallet      --[classic Payment to settlement_G, MEMO_ID=sep24_memo_id]----->  bridge
```

The same `u64` appears in three places: the muxed destination on hop 1, the
SAC `transfer` event data map per CAP-0067, and the classic hop 2
`memo_type=id` on Horizon. Validated end-to-end in
`/home/antb2/dev/rsync/django-polaris-bpv/sep45-reference/python-sep45/test_memo_flow_soroban.py`.

For the implementation footprint and full set of cross-cutting changes
required to add SEP-45 to a Python anchor, see the research memo at
`/home/antb2/dev/rsync/anchor-platform/sep45-research-memo.md`.
